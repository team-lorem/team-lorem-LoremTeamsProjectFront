  ...
  <div class="div_chatCont">
    <div class="massange_cont">
      <div v-for="(msg, index) in messages" :key="index" :class="{'user-message': msg.isUser, 'api-message': !msg.isUser}">
        ...
        <div v-else-if="!msg.isUser && msg.type === 'file'" class="api-message-content">
          <img src="../public/Frame.svg" alt="" class="api-message-content_img"> Response from the API regarding the file: {{ msg.fileName }}
        </div>
      </div>
    </div>
      ...
      <div class="input-container">
        <div class="attachment-icon" @click="triggerFileInput">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="20" viewBox="0 0 14 20" fill="none">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M4 5C4 2.23858 6.2386 0 9 0C11.7614 0 14 2.23858 14 5V13C14 16.866 10.866 20 7 20C3.13401 20 0 16.866 0 13V7C0 6.44772 0.44772 6 1 6C1.55228 6 2 6.44772 2 7V13C2 15.7614 4.23858 18 7 18C9.7614 18 12 15.7614 12 13V5C12 3.34315 10.6569 2 9 2C7.3431 2 6 3.34315 6 5V13C6 13.5523 6.4477 14 7 14C7.5523 14 8 13.5523 8 13V7C8 6.44772 8.4477 6 9 6C9.5523 6 10 6.44772 10 7V13C10 14.6569 8.6569 16 7 16C5.3431 16 4 14.6569 4 13V5Z" fill="white"/>
          </svg>
        </div>
        <input v-model="message" type="text" placeholder="Write a request" />
        <input type="file" ref="fileInput" style="display: none" @change="handleFileUpload" />
        <div class="send-icon" @click="sendMessage">
          <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="20" height="18" viewBox="0 0 20 18" fill="none">
            <path d="M2.35903 6.2606L3.02572 7.4496C3.45015 8.20659 3.66235 8.58501 3.66235 8.99999C3.66235 9.41496 3.45015 9.79338 3.02573 10.5504L2.35903 11.7394C0.460639 15.1251 -0.488558 16.8182 0.251636 17.6577C0.991831 18.4971 2.7446 17.7156 6.25014 16.1526L15.8711 12.2242C18.7454 11.017 20.1825 10.4134 20.1825 8.99999C20.1825 7.58662 18.7454 6.98302 15.8711 5.7758L6.25014 1.84739C2.7446 0.284417 0.991831 -0.497064 0.251636 0.342398C-0.488558 1.18186 0.460639 2.87492 2.35903 6.2606Z" fill="#4C4C4C"/>
          </svg>
        </div>
      </div>
    </div>
  ...
  methods: {
    sendMessage() {
      if (this.message.trim()) {
        ...
        setTimeout(() => {
          this.messages.push({ text: 'Response from the API', isUser: false, type: 'text' });
        }, 1000);
      }
    },
    triggerFileInput() {
    ...
    async handleFileUpload(event) {
      ...
      if (file) {
        ...
        try {
          ...
          this.messages.push({ fileName: file.name, isUser: false, type: 'file' });
        } catch (error) {
